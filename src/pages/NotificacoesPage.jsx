...
function NotificacoesPage({ session }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/notifications`, {
      ...
      .then(async (data) => {
        const visibleNotifications = Array.isArray(data) ? data.filter((notification) => session?.role !== 'CLIENT' || Number(notification.customerId) === Number(session.userId)) : [];
        const technicianIds = [...new Set(visibleNotifications.map((notification) => notification.technicianId).filter(Boolean))];
        const summaries = await Promise.all(technicianIds.map((id) => fetch(`${API_URL}/users/${id}/summary`, { headers: getAuthHeaders() }).then((response) => response.ok ? response.json() : null)));
        setTechnicianNames(Object.fromEntries(summaries.filter(Boolean).map((user) => [user.id, user.name])));
  ...
  function notificationMessage(notification) {
    if (notification.eventType === 'TICKET_ASSIGNED') {
      return `Chamado #${notification.ticketId || 'não informado'} atribuído a ${technicianNames[notification.technicianId] || 'um técnico'}`;
    }
    return notification.message || 'Sem mensagem';
  }
